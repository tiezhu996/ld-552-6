...
export class JobsService {
  ...
  async findOne(id: number, user: any) {
    ...
    if (job && user.role === UserRole.HIRING_MANAGER && job.department !== user.department) throw new ForbiddenException('Access denied');
    return job;
  }
  ...
  async resumes(id: number, user: any) {
    const job = await this.prisma.job.findUnique({ where: { id } });
    if (!job) throw new NotFoundException('Job not found');
    if (user.role === UserRole.HIRING_MANAGER && job.department !== user.department) throw new ForbiddenException('Access denied');
    return this.prisma.resume.findMany({ where: { jobId: id }, include: { candidate: true, interviews: true } });
  }
  async interviews(id: number, user: any) {
    const job = await this.prisma.job.findUnique({ where: { id } });
    if (!job) throw new NotFoundException('Job not found');
    if (user.role === UserRole.HIRING_MANAGER && job.department !== user.department) throw new ForbiddenException('Access denied');
    return this.prisma.interview.findMany({ where: { resume: { jobId: id } }, include: { resume: { include: { candidate: true } }, interviewer: { select: publicUserSelect } } });
  }
}
